...
public class Polis {    //преобразование в полис
    ...
    void var(){
        ...
        while (!token.type.equals("END")){                       //если в правая часть не заканчивается после 1 ого элемента

            token = tokens.poll();
            polis.add(tokens.poll());
            polis.add(token);

            token = tokens.peek(); //возращает верхний элемент tokens в token
